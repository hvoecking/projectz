...
testPath     = pathUtil.join(projectzPath, 'tests', 'dependencies-test')
srcPath      = pathUtil.join(projectzPath, 'tests', 'dependencies-test', 'src')
expectPath   = pathUtil.join(projectzPath, 'tests', 'dependencies-test', 'out-expected')
cliPath      = pathUtil.join(projectzPath, 'bin', 'projectz')

# -------------------------------------
# Tests

joe.suite 'projectz-dependencies', (suite,test) ->
	test 'dependencies', (done) ->

		# Compile with Projectz using -p to switch to the source path.
		command = [cliPath, 'compile', '-p', srcPath, 'dependencies', 'false']
